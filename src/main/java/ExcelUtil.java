...
public class ExcelUtil {
   ...
   private Map<Integer, ArrayList<String>> resultData = new HashMap<>();
    ...
    public  void excelToWord(String excelPath, String wordPath) {
       ...
       this.wordPath = wordPath;
        ...
        try {
            ...
            XSSFSheet sheet = wb.getSheetAt(1); // 获取第一个sheet表
    ...
    public void createWord() {
        ...
        for(int i=1; i <= resultData.size(); i++) {
            if (resultData.get(i).size() > 4) {
                String fileName = i + "-" + resultData.get(i).get(3) + ".doc";
                OutputStream os = null;
                try {
                    boolean flag = copyFile("./template.doc", wordPath + fileName);
                    if (!flag) return;
                    ...
                    range.replaceText("${user}", resultData.get(i).get(0));
                    range.replaceText("${item}", resultData.get(i).get(1));
                    range.replaceText("${code}", resultData.get(i).get(2) + "号");
                    range.replaceText("${price}", resultData.get(i).get(4) + "元");
                    if (resultData.get(i).size() > 5) {
                        range.replaceText("${server_price}", resultData.get(i).get(5) + "元");
                    } else {
                        range.replaceText("${server_price}",   " 元");
                    }

                    if (resultData.get(i).size() > 6) {
                        String date = DateUtil.transform(resultData.get(i).get(6));
